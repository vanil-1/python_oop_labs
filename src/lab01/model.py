...
class House:
    def __init__(self, address: str, floors: int, area: float|int, cost: float|int, min_time_rent: int, rented: bool):
    ...
    def cost(self, value):
        if self._rented:
            ...
            self._cost = validate_cost(value)
            return 'Cost was changed succesfully!'
    ...
    def rented(self, value):
        ...
        if self._rented == value: raise Warning('New value of rented is the same as old!')
        else: self._rented = value
    ...
    def min_time_rent(self, value):
        ...
        if self._min_time_rent == value: raise Warning('New value of minimal time rent is the same as old!')
        else: self._min_time_rent = value
    
    def make_contract(self):
        if self._rented: return 'House had already rented!'
        else:
            self._rented = True
    ...
    def __str__(self):
        header = f"| {'Address':<20} | {'Floors':<6} | {'Area(m²)':<6} | {'Cost($/month)':<14} | {'MinRent(months)':<15} | {'Rented':<6} |"
        separator = "-" * len(header)
        row = f"| {self._address:<20} | {self._floors:<6} | {self._area:<8} | {self._cost:<14} | {self._min_time_rent:<15} | {self._rented!s:<6} |"
        return f"{header}\n{separator}\n{row}"
    ...
    def __eq__(self, other):
        if not isinstance(other, House):
            return False
        return ('Areas are equal. ' if self._area == other._area else f'Areas are not equal! ')\
